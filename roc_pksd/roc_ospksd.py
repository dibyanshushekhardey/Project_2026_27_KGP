...
for sigma in sigma_list:
  p_values_null = []
  p_values_alt = []
  ospksd = OSPKSD(means=[-6, 6], sigmas=[sigma, sigma], weights=[0.5, 0.5],jump_ls=jump_ls,T=T, num_starts=num_starts,device=device,)

  for trial in range(n_trials):
    # Null
    train_null = sample_gmm(n=n, means=[-6, 6], sigmas=[sigma, sigma],weights=[0.5, 0.5],device=device)
    best_jump_null = ospksd.find_best_jump(train_null)
    test_null = sample_gmm( n=n,means=[-6, 6], sigmas=[sigma, sigma],weights=[0.5, 0.5], device=device)
    xt_null = ospksd.perturb_samples(test_null, best_jump_null)
    #scale0 = median_heuristic(x_null)
    ksd0, U0 = ospksd.compute_ospksd(test_null,xt_null)
    boot_stats_0 = torch.stack([ospksd.bootstrap_stat(U0) for _ in range(num_boot)])
    ...
    xt_alt = ospksd.perturb_samples(test_alt, best_jump_null)
    #scale1 = median_heuristic(x_alt)
    ksd1, U1 = ospksd.compute_ospksd(test_alt,xt_alt,)
    boot_stats_1 = torch.stack([ospksd.bootstrap_stat(U1) for _ in range(num_boot)])
...
plt.title(f"ROC Curves - ospKSD Bootstrap Test\nn={n}, shift=1.0, num_boot={num_boot}", fontsize=14)
plt.legend(loc='lower right', fontsize=9)
plt.grid(True, alpha=0.3)
plt.xlim([0, 1])
plt.ylim([0, 1])
plt.tight_layout()
plt.savefig('roc_curve_ospKSD.png', dpi=300, bbox_inches='tight')
plt.show()
...
summary_df.to_csv("ospksd_summary.csv", index=False)
roc_df.to_csv("ospksd_roc.csv", index=False)
trial_df.to_csv("ospksd_trial.csv", index=False)