...
for weight in weight_list:
  ...
  for trial in range(n_trials):
      ...
      x_ksd = sample_gmm(n=n,means=[-delta/2, delta/2],sigmas=[sigma, sigma], weights=[weight, 1.0-weight],device=device,)      # all samples from right mode
  ...
  for trial in range(n_trials):
      # Sample distribution p (ratio_s = 1.0)
      x_pksd = sample_gmm(n=n,means=[-delta/2, delta/2],sigmas=[sigma, sigma], weights=[weight, 1.0-weight],device=device,)      # all samples from right mode
  ...
  for trial in range(n_trials):
      # Sample distribution p (ratio_s = 1.0)
      x_ospksd = sample_gmm(n=n,means=[-delta/2, delta/2],sigmas=[sigma, sigma], weights=[weight, 1.0-weight],device=device,)      # all samples from right mode
  ...
  for trial in range(n_trials):
      # Sample distribution p (ratio_s = 1.0)
      x_spksd = sample_gmm(n=n,means=[-delta/2, delta/2],sigmas=[sigma, sigma], weights=[weight, 1.0-weight],device=device,)      # all samples from right mode
...
plt.plot(weight_list,rejection_rates_ksd,marker='o',linewidth=2,label='KSD')
plt.plot(weight_list,rejection_rates_pksd,marker='x',linewidth=2,label='PKSD')
plt.plot(weight_list,rejection_rates_ospksd,marker='d',linewidth=2,label='OSPKSD')
plt.plot(weight_list,rejection_rates_spksd,marker='h',linewidth=2,label='SPKSD')
...
# Save the figure
plt.savefig("rejection_rate_weights.png", dpi=300, bbox_inches="tight")
